...
const MyLibrary: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"bookmarks" | "follows">(
    "bookmarks",
  );
  ...
  const fetchLibraryData = async () => {
    ...
    setError(null);
    try {
      // Đã cập nhật tên hàm thành getBookmarks và getFollows
      const [bookmarksRes, followsRes] = await Promise.all([
        api.getBookmarks(),
        api.getFollows(),
      ]);

      // Xử lý an toàn dữ liệu trả về (phòng hờ backend trả về trực tiếp mảng hoặc bọc trong thuộc tính data)
      const bItems = (bookmarksRes as any)?.data || bookmarksRes;
      const fItems = (followsRes as any)?.data || followsRes;

      setBookmarks(Array.isArray(bItems) ? bItems : bItems?.items || []);
      setFollows(Array.isArray(fItems) ? fItems : fItems?.items || []);
    } catch (err: any) {
  ...
  const handleRemoveBookmark = async (
    ...
    try {
      await api.toggleBookmark(targetId, targetType);
      // Xóa item khỏi danh sách hiển thị ngay lập tức để UI phản hồi nhanh
      setBookmarks((prev) =>
        prev.filter(
  ...
  const handleUnfollow = async (
    ...
    try {
      ...
      alert("Lỗi khi hủy theo dõi. Vui lòng thử lại!");
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-40">
        ...
        <p className="text-sm text-[#74777f] mt-4">
          Đang tải thư viện cá nhân của bạn...
        </p>
      </div>
    );
  }
  ...
  const paperBookmarks = bookmarks.filter(
    (b) => b.targetType.toLowerCase() === "paper",
  );
  const keywordBookmarks = bookmarks.filter(
    (b) => b.targetType.toLowerCase() === "keyword",
  );
  const authorFollows = follows.filter(
    (f) => f.targetType.toLowerCase() === "author",
  );
  const journalFollows = follows.filter(
    (f) => f.targetType.toLowerCase() === "journal",
  );
  const topicFollows = follows.filter(
    (f) => f.targetType.toLowerCase() === "researchtopic",
  );
    ...
    <div className="space-y-6 animate-fadeIn max-w-5xl mx-auto">
      ...
      <div>
        ...
        <p className="text-xs text-[#74777f] mt-1">
          Quản lý toàn bộ tài liệu nghiên cứu đã lưu trữ và các đối tượng đang
          theo dõi
        </p>
      </div>
                  ...
                  <div className="flex flex-wrap gap-3 p-4 bg-white border border-[#ebeef0] rounded-lg shadow-sm">
                    {keywordBookmarks.map((b) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            handleRemoveBookmark(b.targetId, "Keyword")
                          }
        ...
        <div className="space-y-6">
          {follows.length === 0 ? (
            <div className="text-center py-20 bg-white border border-[#ebeef0] rounded-lg text-[#74777f]">
              ...
              <p className="text-sm">
                Bạn chưa theo dõi tác giả, tạp chí hoặc chủ đề nào.
              </p>
              <Link
                to="/search"
                className="mt-4 inline-flex items-center gap-1 bg-[#13696a] hover:opacity-95 text-white px-5 py-2.5 rounded text-xs font-bold transition-all"
                    ...
                    {authorFollows.map((f) => (
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-[#74777f] mt-1">
                            Tổng số công bố:{" "}
                            <strong>{f.paperCount ?? 0} bài báo</strong>
                          </p>
                        </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {topicFollows.map((f) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            handleUnfollow(f.targetId, "ResearchTopic")
                          }
                          className="border border-[#c4c6cf] text-[#43474e] hover:bg-red-50 hover:text-red-700 hover:border-red-200 py-1.5 px-3 rounded text-xs font-bold transition-all cursor-pointer flex items-center gap-1"
                        >
                          <span className="material-symbols-outlined text-xs">
