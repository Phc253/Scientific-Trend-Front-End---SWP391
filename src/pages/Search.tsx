...
const Search: React.FC = () => {
  ...
  // Khách mặc định dùng theme màu xanh dương (như Sinh viên)
  const theme = {
  ...
  // Lấy từ khóa từ URL nếu người dùng gõ từ ô tìm kiếm ở Header
  ...
  // 1. KHÔI PHỤC TRẠNG THÁI TỪ SESSION STORAGE
  useEffect(() => {
    const savedState = sessionStorage.getItem("guestSearchState");
    if (savedState) {
      try {
        const parsed = JSON.parse(savedState);
        setActiveTab(parsed.activeTab || "papers");
        setSearchQuery(parsed.searchQuery || searchParams.get("q") || "");
  ...
  const fetchData = async (currentPage: number, currentTab: SearchTab) => {
    ...
    try {
      if (currentTab === "papers") {
        ...
        // Áp dụng fix lỗi đồng bộ dữ liệu
  ...
  // 3. XỬ LÝ SỰ KIỆN FORM & PHÂN TRANG
  ...
  // 4. CHẶN THAO TÁC CẦN ĐĂNG NHẬP
  const handleRequiresAuth = () => {
    if (
      window.confirm(
        "Bạn cần đăng nhập để sử dụng tính năng Lưu và Theo dõi. Chuyển đến trang Đăng nhập?",
      )
    ) {
      navigate("/login");
    }
        ...
        {activeTab === "papers" ? (
          ...
          <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm h-fit space-y-4">
            ...
            <p className="text-sm text-slate-500 leading-relaxed">
              Tạo tài khoản miễn phí để sử dụng tính năng "Theo dõi" và "Lưu
              lại". Hệ thống sẽ cá nhân hóa bảng tin dành riêng cho bạn!
            </p>
          </div>
        )}
                  ...
                  >
                    <div className="flex-1">
                      <Link
                        ...
                        className={`text-lg font-bold text-slate-800 hover:text-blue-600 transition-colors block mb-2 leading-snug`}
                      >
                        {paper.title}
                      </Link>
                    ...
                    <div className="flex sm:flex-col gap-2 justify-start sm:justify-center shrink-0 border-t sm:border-t-0 sm:border-l border-slate-100 pt-3 sm:pt-0 sm:pl-4">
                      ...
                      <button
                        ...
                        className="flex items-center justify-center gap-1.5 px-4 py-2 rounded-lg border font-bold text-sm transition-colors w-full sm:w-auto bg-white border-slate-200 text-slate-600 hover:bg-slate-50"
                      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {facetItems.map((item) => (
                ...
                >
                  ...
                  <button
                    onClick={handleRequiresAuth}
                    className="text-xs font-bold py-2 px-3.5 rounded-lg flex items-center gap-1.5 transition-all cursor-pointer border border-slate-200 text-slate-600 hover:bg-slate-50"
                  >
                    <span className="material-symbols-outlined text-[14px]">
                      {activeTab === "keywords" ? "bookmark_add" : "person_add"}
                    </span>
                    {activeTab === "keywords" ? "Lưu từ khóa" : "Theo dõi"}
                  </button>
                </div>
              ))}
            </div>
