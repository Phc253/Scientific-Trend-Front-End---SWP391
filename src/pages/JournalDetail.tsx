...
import { api } from "../services/api"; // Điều chỉnh lại đường dẫn import api cho đúng
import type { Paper } from "../services/api";

const JournalDetail: React.FC = () => {
  // Lấy tên tạp chí từ URL (chú ý: tên tạp chí có thể chứa ký tự đặc biệt nên sẽ được encode)
  const { journalName } = useParams<{ journalName: string }>();
  const navigate = useNavigate();

  const decodedName = journalName ? decodeURIComponent(journalName) : "";
  ...
  const isResearcher = role.includes("Researcher"); // Hỗ trợ thêm role Researcher như bạn mong muốn
  ...
  if (isLecturer) {
    ...
    primaryColor =
      "bg-purple-600 hover:bg-purple-700 text-purple-600 border-purple-600"; // Định hình màu tím cho nhà nghiên cứu
    basePath = "/researcher";
  }

  // States quản lý bài báo và phân trang
  ...
  useEffect(() => {
    if (decodedName) {
      fetchPapersByJournal(page);
    }
  }, [page, decodedName]);

  const fetchPapersByJournal = async (currentPage: number) => {
    setIsLoading(true);
    setError(null);
    try {
      // Gọi API searchPapers và truyền tên tạp chí vào bộ lọc chuyên biệt
      ...
      setError(
        err.message || "Không thể tải danh sách bài báo thuộc tạp chí này.",
      );
    } finally {
      setIsLoading(false);
    }
            ...
            {papers.map((paper) => (
              ...
              >
                <div className="flex-1">
                  {/* Điều hướng thông minh dựa vào việc đã đăng nhập hay chưa */}
                  <Link
                    to={
                      basePath === "/"
                        ? `/paper/${paper.paperId}`
